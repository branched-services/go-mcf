I need you to decompose the specification in `docs/spec.md` into a set of Flywheel task definitions that will be executed by parallel AI coding agents. Read the spec thoroughly first, then produce JSON task files in `.flywheel/tasks/`, one file per task.
